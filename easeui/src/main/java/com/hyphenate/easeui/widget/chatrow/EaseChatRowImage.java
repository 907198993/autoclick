...
public class EaseChatRowImage extends EaseChatRowFile{
    ...
    protected void onSetUpView() {
        imageSize=DensityUtil.dip2px(context, 152);
    ...
    private void showImageView(final String thumbernailPath, final String localFullSizePath,final EMMessage message) {
      /*  // first check if the thumbnail image already loaded into cache s
        ...
        Glide.with(activity).load(message.direct()== EMMessage.Direct.SEND?localFullSizePath:thumbernailPath).apply(new RequestOptions().placeholder(R.drawable.ease_default_image).error(R.drawable.ease_default_image).override(imageSize, imageSize).dontAnimate()).into(imageView);
    }
