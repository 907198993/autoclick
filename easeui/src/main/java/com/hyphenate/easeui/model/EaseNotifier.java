...
public class EaseNotifier {
    ...
    void cancelNotification() {
        if (notificationManager != null){
            ...
            {
                notificationManager.cancel(notifyIds.get(key));
            }
        }
        notifyIds.clear();
    }
    ...
    public synchronized void notify(EMMessage message) {
        ...
        if (!EasyUtils.isAppRunningForeground(appContext)) {
            ...
            if (!notifyIds.containsKey(message.getFrom())){
                notifyIds.put(message.getFrom(),random.nextInt(Integer.MAX_VALUE));
                Log.i("notifyIds","添加notifyId");
            }
    ...
    protected void handleMessage(EMMessage message) {
        try {
            ...
            if (notificationInfoProvider != null) {
                ...
                if (i != null) {
                    PendingIntent pendingIntent = PendingIntent.getActivity(appContext, notifyIds.get(message.getFrom()), i, PendingIntent.FLAG_UPDATE_CURRENT);
                    builder.setContentIntent(pendingIntent);
                }
                int messageCount=EMClient.getInstance().chatManager().getConversation(message.getFrom()).getUnreadMsgCount();
                notifyText = notificationInfoProvider.getLatestText(message, fromUsersNum, messageCount);
                if (notifyText != null) {
            ...
            Notification notification = builder.build();
            notificationManager.notify(notifyIds.get(message.getFrom()), notification);
